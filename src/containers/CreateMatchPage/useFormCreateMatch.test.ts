...
describe('useFormCreateMatch', () => {
	...
	describe('handleChange', () => {
		it('should update name field and clear error on valid input', () => {
			...
			act(() => {
				result.current.handleChange({ target: { name: 'name', value: 'Valid Name' } } as ChangeEvent<HTMLInputElement>)
			})
		...
		it('should set an error when min_players is out of range', () => {
			...
			act(() => {
				result.current.handleChange({
					target: { name: 'min_players', value: (RANGE_PLAYERS.MIN - 1).toString() },
				} as ChangeEvent<HTMLInputElement>)
			})

			expect(result.current.formError.min_players).toBe(ERROR_MESSAGES.MIN_PLAYERS_OUT_RANGE)
			expect(result.current.haveError).toBeTruthy()
		})
		...
		it('should set an error when max_players is out of range', () => {
			...
			act(() => {
				result.current.handleChange({
					target: { name: 'max_players', value: (RANGE_PLAYERS.MAX + 1).toString() },
				} as ChangeEvent<HTMLInputElement>)
			})

			expect(result.current.formError.max_players).toBe(ERROR_MESSAGES.MAX_PLAYERS_OUT_RANGE)
			expect(result.current.haveError).toBeTruthy()
		})
		...
		it('should set an error when min_players is greater than max_players on change', () => {
			...
			act(() => {
				...
				result.current.handleChange({
					target: { name: 'min_players', value: (curr_max_players + 1).toString() },
				} as ChangeEvent<HTMLInputElement>)
			})

			expect(result.current.formError.min_players).toBe(ERROR_MESSAGES.MIN_PLAYERS_GREATER_MAX_PLAYERS)
			expect(result.current.haveError).toBeTruthy()
		})
		...
		it('should set an error when max_players is less than min_players on change', () => {
			...
			act(() => {
				...
				result.current.handleChange({
					target: { name: 'max_players', value: (curr_min_players - 1).toString() },
				} as ChangeEvent<HTMLInputElement>)
			})

			expect(result.current.formError.max_players).toBe(ERROR_MESSAGES.MAX_PLAYERS_LESS_MIN_PLAYERS)
			expect(result.current.haveError).toBeTruthy()
		})
	})
	...
	describe('createHandleSubmit', () => {
		it('should not call handleCreateMatch if there are errors', async () => {
			...
			await act(async () => {
				const handleSubmit = result.current.createHandleSubmit(mockHandleCreateMatch)
		...
		it('should handle errors from handleCreateMatch gracefully', async () => {
			...
			const mockHandleCreateMatch = vi.fn().mockRejectedValueOnce(new Error('API error'))
