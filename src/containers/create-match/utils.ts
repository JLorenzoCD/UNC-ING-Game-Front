...
const validateMinMaxRelation = (min: number, max: number): { min_players: string; max_players: string } => {
	if (max < min) {
		return {
...
export const validateForm = (data: MatchForm): MatchFormError => {
	...
	const minMaxRelationErrors = validateMinMaxRelation(parsedMinPlayers, parsedMaxPlayers)
