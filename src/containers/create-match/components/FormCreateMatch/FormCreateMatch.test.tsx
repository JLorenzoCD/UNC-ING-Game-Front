...
describe("FormCreateMatch", () => {
  ...
  it("should render the form with initial values and no errors", () => {
    ...
    expect(
      screen.queryByTestId("mock-alert-error-list"),
  ...
  it("should show an error when min_players is greater than max_players on submission", async () => {
    ...
    expect(
      screen.getByText(ERROR_MESSAGES.MIN_PLAYERS_GREATER_MAX_PLAYERS),
    ).toBeInTheDocument();
    expect(
      screen.getByText(ERROR_MESSAGES.MAX_PLAYERS_LESS_MIN_PLAYERS),
    ).toBeInTheDocument();
  });
});
